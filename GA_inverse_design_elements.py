...
target_index = 10
target_index = np.loadtxt(str(number)+"_test_index.csv")
target = data[:,134:].astype(np.float32)
target_spectrum = target[int(target_index)]
elements =['Ag', 'Al', 'Ba', 'Bi', 'Ca', 'Ce', 'Co', 'Cr', 'Cu', 'Er', 'Eu', 'Fe', 'Ga', 'Gd', 'Hf', 'In', 'La', 'Lu', 'Mg', 'Mn', 'Mo', 'Nb', 'Nd', 'Ni', 'P', 'Pb', 'Pd', 'Pr', 'Rb', 'Sb', 'Sc', 'Sm', 'Sn', 'Sr', 'Tb', 'Ti', 'V', 'W', 'Y', 'Yb', 'Zn', 'Zr']

ratios = np.arange(0.01,1.0,0.01)
gene_length = 13 #6 for elements and 7 for ratios
element_count = 3
...
def evalresult(individual):
    ...
    for i in range(element_count):
        ...
        element_i=binary2dec(gene[0:6])
        ratio_i=binary2dec(gene[6:])
        if element_i == 0:
            continue
        else:
            element = elements[(element_i-1)%len(elements)]
...
def main():
    ...
    with tf.Session(
        ...
        np.savetxt(str(target_index)+"_"+ str(number)+"_ga_best_spectrum_elements.csv", best_spectrum, delimiter = ",")
